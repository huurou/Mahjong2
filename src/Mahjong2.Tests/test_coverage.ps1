...
if (-not (Test-Path -Path ".\TestResults")) {
    Write-Error "TestResultsディレクトリが見つかりません。テストが正常に実行されなかった可能性があります。"
    exit 1
}
...
if ($coverageFiles.Count -eq 0) {
    Write-Error "カバレッジファイルが見つかりません。テスト実行中に問題が発生した可能性があります。"
    exit 1
}
...
try {
    ...
    Write-Error "reportgeneratorの実行中にエラーが発生しました。ReportGenerator(.NET Global Tool)がインストールされているか確認してください。"
    Write-Error "インストールするには: dotnet tool install -g dotnet-reportgenerator-globaltool"
    exit 1
}
...
if (-not (Test-Path $reportPath)) {
    Write-Error "生成されたレポートファイルが見つかりません: $reportPath"
    exit 1
}
